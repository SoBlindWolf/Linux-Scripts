while true; do
    ...
    case $yn in
        [Yy]* ) sudo LC_ALL=C.UTF-8 add-apt-repository ppa:ondrej/php -y && sudo apt-get update && sudo apt-get upgrade -y && sudo apt-get purge php5-common -y && sudo apt-get remove php5-common -y && sudo apt-get install screen wget apache2 apache2-utils python-software-properties php7.0 php7.0-mcrypt php7.0-bz2 mysql-client php7.0-curl libapache2-mod-php7.0 build-essential libgd2-xpm-dev curl openssh-server mysql-server php7.0-mysql openssl -y && sudo service apache2 restart && sudo apt-get autoremove -y && sudo apt-get clean -y; break;;
        [Nn]* ) break;;
