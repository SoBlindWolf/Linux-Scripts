...
while true; do
    ...
    case $yn in
        [Yy]* ) sudo mv /etc/nginx/nginx.conf /etc/nginx.conf.orig && sudo wget -O /etc/nginx/ https://raw.githubusercontent.com/SoBlindWolf/Linux-Scripts/master/configs/nginx/nginx.conf && sudo mv /etc/php/7.0/fpm/php.ini /etc/php/7.0/fpm/php.ini.orig && sudo wget -O /etc/php/7.0/fpm/ https://raw.githubusercontent.com/SoBlindWolf/Linux-Scripts/master/configs/php-fpm/php.ini && sudo mv /etc/php/7.0/fpm/pool.d/www.conf /etc/php/7.0/fpm/pool.d/www.conf.orig && sudo wget -O /etc/php/7.0/fpm/pool.d/ https://raw.githubusercontent.com/SoBlindWolf/Linux-Scripts/master/configs/php-fpm/www.conf && sudo mv /etc/php/7.0/fpm/php-fpm.conf /etc/php/7.0/fpm/php-fpm.conf.orig && sudo wget -O /etc/php/7.0/fpm/ https://raw.githubusercontent.com/SoBlindWolf/Linux-Scripts/master/configs/php-fpm/php-fpm.conf && sudo service php7.0-fpm restart && sudo service nginx restart; break;;
        [Nn]* ) exit;;
        * ) echo "Please answer yes or no.";;
    esac
done
